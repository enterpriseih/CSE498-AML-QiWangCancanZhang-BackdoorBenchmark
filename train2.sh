...
if [ "$attack" = "BppAttack" ]
then 
    python -u model/train/$attack.py --dataset $dataset --attack_mode all2one --squeeze_num 32 --data_root 'datasets/' --checkpoints "checkpoints/${attack}/" --device $device --n_iters $n_iters --bs $batch_size
elif [ "$attack" = "WaNet" ]
then
